...
void RenderFlip(u32 nSync)
{
	...
	if (nSync) FrameWait();
	SDL_Surface* convertedSurface = SDL_ConvertSurfaceFormat(gVar.pScreen, SDL_PIXELFORMAT_RGB888, 0);
	SDL_UpdateTexture(gRender.sdlTexture, NULL, convertedSurface->pixels, convertedSurface->pitch);
	SDL_RenderClear(gRender.sdlRenderer);
	SDL_RenderCopy(gRender.sdlRenderer, gRender.sdlTexture, NULL, NULL);
	SDL_RenderPresent(gRender.sdlRenderer);
...
void Render_InitVideo(void)
{
	...
	gRender.sdlWindow = SDL_CreateWindow("Mini Slug Project by 17o2!!",
                             SDL_WINDOWPOS_UNDEFINED,
                             SDL_WINDOWPOS_UNDEFINED,
                             SCR_Width, SCR_Height,
                             SDL_WINDOW_ALLOW_HIGHDPI);

	if (gRender.sdlWindow == NULL)
	{
		fprintf(stderr, "Render_InitVideo(): Couldn't set video mode: %s\n", SDL_GetError());
	}

	gRender.sdlRenderer = SDL_CreateRenderer(gRender.sdlWindow, -1, 0);

	gVar.pScreen = SDL_CreateRGBSurfaceWithFormat(0, SCR_Width, SCR_Height, 16, SDL_PIXELFORMAT_RGB565);

	if (gVar.pScreen == NULL) exit(1);

    gRender.sdlTexture = SDL_CreateTexture(gRender.sdlRenderer,
...
void CheatCodes_KeyPressed(u32 nSdlKey)
{
	...
	if (nSdlKey < SDL_SCANCODE_A || nSdlKey > SDL_SCANCODE_Z) return;
...
int EventHandler(u32 nInGame)
{
	...
	while (SDL_PollEvent(&event))
	{
		...
		{
		case SDL_KEYDOWN:
			gVar.pKeys[event.key.keysym.scancode] = 1;

			// Pas en cours de jeu ?
			if (nInGame == 0)
			{
				CheatCodes_KeyPressed(event.key.keysym.scancode);	// Test des cheat codes.
...
// Sortir apr�s la boucle ? => pas s�r.
			...
			if (gVar.pKeys[SDL_SCANCODE_F10])
			{
				...
				if (gRender.nFullscreenMode)
				{
					SDL_SetWindowFullscreen(gRender.sdlWindow, SDL_WINDOW_FULLSCREEN_DESKTOP);
				}
				else
				{
					SDL_SetWindowFullscreen(gRender.sdlWindow, 0);
				}
			}

#ifdef	DEBUG_KEYS
			if (gVar.pKeys[SDL_SCANCODE_ESCAPE]) return (1);	// Emergency exit.
#endif
			break;
...
if (gVar.pKeys[SDL_SCANCODE_Y]) { FrameWait(); FrameWait(); FrameWait(); FrameWait(); FrameWait(); FrameWait(); }
#endif
...
#define	PAUSE_KEY	(SDL_SCANCODE_F12)
...
*/
	...
	while (1)
	{
		...
		// Button A.
		if (gVar.pKeys[SDL_SCANCODE_RETURN] || gVar.pKeys[SDL_SCANCODE_SPACE] || gVar.pKeys[gMSCfg.pKeys[e_CfgKey_ButtonA]])
		{
...
_JoyOff:
	...
	// Init du clavier.
	gVar.pKeysSDL = SDL_GetKeyboardState(NULL);
	memset(gVar.pKeys, 0, SDL_NUM_SCANCODES);
...
//if (gVar.pKeysSDL[SDL_SCANCODE_RETURN])
//	printf("hidden part!!!\n");
