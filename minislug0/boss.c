...
/*
// Version qui renvoie le dernier d�gat. Ok, mais voir si �a sert � quelque chose.

// Idem Mst_ShotCheckLife mais en passant un rectangle au lieu d'un sprite.
// Out: 0 = Le monstre n'est pas mort / !0 = Le dernier d�gat (utile pour le type de d�gats).
...
//=============================================================================
// Boss 1 : Avion �cras� (MS1 - Mission 1).
...
struct SMst20_CannonUp
{
	...
	s16	nOffsY;
	u8	nShotCnt;
	u8	nShotNo;			// Pour comptage des tirs et d�calage.
...
struct SMst20_Boss1
{
	...
	//
	u8	nWaitItm;			// Item de qu�te attendu pour d�marrer.
	u8	nHitCnt;
	u8	nDecayState;		// Etat de d�labrement [0;4].
	u8	nLife;
	u8	nDeathCnt;
	u8	nSoldierAppCnt;		// Compteur pour ne pas faire apparaitre les soldats trop vite.
	u8	nExplosions;		// Compteur pour pas de touch� pendant les explosions.
...
#define	BOSS1_ENERGY	30//25//20//4//50	// Vie, * nombre d'�tats.
...
void Mst20_CannonUp_Manage(struct SMstCommon *pMst, u16 nPrio)
{
	...
	{
	case e_MstB1_CannonUp_Wait:		// Attente cach�.
	...
	case e_MstB1_CannonUp_Shot:		// En phase de tir.
		// D�clenchement de l'anim de tir ?
	...
	case e_MstB1_CannonUp_GoDown:	// Cass�, descente.
...
void Mst20_CannonFront_Init(struct SMst20_CannonUp *pCannonFront)
{
	...
	pCannonFront->nShotHole = 0;		// b7 = Sens de d�placement, b0-3 compteur.
}
...
// Supprime les collisions des blocs demand�s.
...
// Avance de la phase.
	//>>> m�thode pas forc�ment d�finitive, on pourrait faire �a en fct des points de vie.
...
{
	struct SMst20_Boss1	*pSpe = (struct SMst20_Boss1 *)pMst->pData;
	// Table de tous les sprites composant le boss, 5 �tats.
	static u32	gpBoss1Spr[][BOSS1_PIECESNB] =
	{
		...
		// Le dernier �tat ci-dessous est identique au pr�c�dent.
		{ e_Spr_Boss1_P1_2345, e_Spr_Boss1_P2_345, e_Spr_Boss1_P3_12345, e_Spr_Boss1_P4_345, e_Spr_Boss1_P5_5,
			SPR_NoSprite, e_Spr_Boss1_P7_5, SPR_NoSprite, e_Spr_Boss1_P9_45, e_Spr_Boss1_P10_2345 },
	};
	...
	switch (pMst->nPhase)
	{
	case e_MstB1_ItemWait:		// Attente de l'item de qu�te.
	...
	case e_MstB1_Intro:			// Mont�e du scroll.
		// Recentrage du scroll en y au d�but.
	...
	case e_MstB1_Fight:
//		nTouch = 1;				// Prise en compte des touch�s.
		nTouch = !pSpe->nExplosions;	// Prise en compte des touch�s, sauf pendant les explosions.
...
*/
			...
			if (((++nSide) & 1) == 0)
			{
				...
				nPrm |= ((-8 & 0x3F) << 17);	// (x & 0x3F) << 17 = d�calage org.
...
//<<<
	...
	// Se prend un tir ?
	{
		...
		// On cr�� un rectangle de collision.
		...
		if (nTouch)
		{
			// Test des tirs, avec d�gats minimis�s.
			if (Mst_ShotCheckLifeRect2(&sRect1, pMst->nPosX, pMst->nPosY, &pSpe->nHitCnt, &pSpe->nLife, e_ShotCheck_MinimizedDamage))
			{
				...
				pSpe->nExplosions = 64;		// Compteur pour pas de touch� pendant les explosions.
				pSpe->nLife = BOSS1_ENERGY;		// Reset energy.
				pSpe->nHitCnt = MST_HIT_CNT;	// Pas vraiment mort, on met le hit cnt.
				// Mort ?
				if (pSpe->nDecayState >= BOSS1_DECAY_MAX)
				{
					gpMstQuestItems[MST_QUEST_ITEM_NEXT_LEVEL] |= 2;	// Item de qu�te : Victory.
	...
	if (pSpe->nExplosions) pSpe->nExplosions--;		// Compteur pour pas de touch� pendant les explosions.
...
struct SMst20_Boss2A
{
	u8	nReserved_Boss0;	// Pour mappage sur le Mst20 container.
	//
	u8	nDecayState;		// Etat de d�labrement [0;1].
	u8	nHitCnt;
	u8	nLife;
...
//			if (Mst_ShotCheckLife(gpnFrontCBaseSpr[i] + nInc, pMst->nPosX + (B2A_FRONTCANNON_OFFSX << 8), pMst->nPosY + (gpnFrontCPosY[i] << 8), &pFC->nHitCnt, &pFC->nLife))
			...
			{	// Le canon n'est pas mort.
				ChaserTarget_AddToList((pMst->nPosX >> 8) + B2A_FRONTCANNON_OFFSX, (pMst->nPosY >> 8) + gpnFrontCPosY[i] - 16);	// Ajout dans la table de vis�e des missiles.
...
//			if (Mst_ShotCheckLife(e_Spr_Boss2Front_Turret, pMst->nPosX + (gpnTurretPosX[i] << 8), pMst->nPosY + (gpnTurretPosY[i] << 8), &pTurret->nHitCnt, &pTurret->nLife))
		...
		if (pTurret->nShotAnm != -1)
		{
			if (AnmGetImage(pTurret->nShotAnm) == -1)
				pTurret->nShotAnm = -1;		// Anim termin�e.
			else
			...
			{	// D�clenchement du tir.
				...
				{
					s32	x1, y1, x2, y2;
					if ((((pTurret->nAngle + 4) & 0xFF) / 64) & 1)		// On coupe en 4, �a marche aussi pour les rectangles (plats) des angles 0, 64, 128 et 192.
...
// Arriv�e du boss (A et B), ralentissement sur la fin du mouvement.
...
s32 Mst20_Main_Boss2A(struct SMstCommon *pMst)
{
	...
	// Table de tous les sprites composant le boss, 2 �tats.
	...
	u8	nTurretTouch = 0;	// Gestion du touch� sur les tourelles.
...
*/
	...
	// D�placement pendant les phases d'avance.
	if (pMst->nPhase == e_MstB2A_Avance1 || pMst->nPhase == e_MstB2A_Avance2)
		Boss2_MoveIn(pMst, -1);

	switch (pMst->nPhase)
	{
	case e_MstB2A_ComeDistance:	// L'avion traverse l'�cran au loin.
		// Display.
...
//if (gVar.pKeys[SDL_SCANCODE_G]) { pSpe->sFrontC[0].nLife = 0; pSpe->sFrontC[1].nLife = 0; gVar.pKeys[SDL_SCANCODE_g] = 0; }
		...
		if (gShoot.nDeathFlag) pSpe->nShotFrm = 32;		// Si h�ros mort, stoppe les tirs.
...
//if (gVar.pKeys[SDL_SCANCODE_G]) { pSpe->sTurret[0].nLife = 0; pSpe->sTurret[1].nLife = 0; gVar.pKeys[SDL_SCANCODE_g] = 0; }
		...
		nTurretTouch = 1;	// Gestion du touch� sur les tourelles.
		pSpe->nShotFrm++;	// Frame cnt pour les tirs.
	...
	case e_MstB2A_Recule2:
		...
		if (pMst->nPosX >= gScrollPos.nPosX + ((SCR_Width + 72) << 8))
		{
			...
			// On rel�che le scroll.
			gpMstQuestItems[MST_QUEST_ITEM_MST32]--;
/*
			// On g�n�re le Boss 2B.
			{
				/ *
				[mst20] Boss
				No = 0:3: Boss1 - Boss2A - Boss2B -
				Wait itm = 8:15:		; Item de qu�te attendu.
				RESERVED1 = 16:31:
				* /
				u32	nPrm = 2;
...
//	if (pMst->nPhase >= e_MstB2A_Avance2 && pMst->nPhase <= e_MstB2A_Phase2)
	...
	// Les pilotes.
	static u8 pnPilotsAdd[] = { 6,5,4, 0, 7,8,9, 0,1,2,3 };		// 0-2: Haut > neutre / 3: neutre / 4-6: neutre > Bas // 7-10: neutre > arri�re.
...
*/
	...
	switch (pMst->nPhase)
	{
	case e_MstB2B_ComeDistance:	// L'avion traverse l'�cran au loin.
		// Display.
		i = (gnFrame >> 2) & 3;
...
//			gpMstQuestItems[MST_QUEST_ITEM_MST32]++;	// Blocage du scroll.
	...
	case e_MstB2B_Phase1:		// Flammes.
		...
		// Restart du tir quand l'anim arrive � la fin.
		// Fait comme �a car l'anim qui boucle pose un probl�me lors de la coupure (l'anim de flammes n'est pas interrompue sur la fin, seulement si il y a un rectangle de col).
	...
	case e_MstB2B_Death:		// Mort.
		...
		if (pSpe->nExplo < 60) gpMstQuestItems[MST_QUEST_ITEM_NEXT_LEVEL] |= 1;	// Item de qu�te : Fin de niveau.
		break;
	}
	...
	// Touch� du joueur avec les flammes.
...
void B3C_Manage(struct SMstCommon *pMst)
{
	...
	case e_MstB3_Cannon_Move:
		...
		if (MST_SHOT_COND == 0)		// Ne tire pas quand h�ros mort.
			pCan->nShotCnt = -1;
		else
...
void B3U_Manage(struct SMstCommon *pMst)
{
	...
	case e_MstB3_Ufo_Arrival:	// Arriv�e.
		...
		if (nDiff < 0x40)
		{
			...
			pSpe->sB3Cannon.nPhase = e_MstB3_Cannon_Move;	// D�marrage du canon.
		}
		break;
	...
	if (Mst_ShotCheckLife2(e_Spr_MiniUFO_Fly, pUfo->nPosX, pUfo->nPosY, &pUfo->nHitCnt, &pUfo->nLife, e_ShotCheck_MinimizedDamage))
	{
		...
		if (++pSpe->nDecayState >= BOSS3_DECAY_MAX)
		{
			...
			FireRemoveFromList((1<<31) | e_ShotOrg_Enemy);
			//
			pSpe->sB3Cannon.nPhase = e_MstB3_Cannon_Wait;	// Arr�t du canon (et du tir en cours).
...
//		nSpr2 = (pMst->nAnm == -1 ? e_Spr_Core_CoreGlow : AnmGetImage(pMst->nAnm));
		// Glow + truc de lamer pour acc�l�rer l'anim.
	...
	case e_MstB3_Dead:	// Mort.
		// Apr�s la derni�re explosion....
		if (pSpe->nExplo == 1)
		{
			// On g�n�re un Monoeye (place holder).
			/*
			[mst48] Jokes
