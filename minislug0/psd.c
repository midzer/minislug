...
// D�prot�ge un fichier PSD si n�cessaire.
void PSD_Unprotect(u8 *pBuf)
{
	char	pNorm[4] = { '8'^0x80,'B'^0x81,'P'^0x82,'S'^0x83 };		// Avec un xor pour masquer la cha�ne dans l'exe.
...
struct SPSDPicture * PSDLoad(char *pPSDFilename)
{
	...
	PSD_Unprotect(pBuf);	// Si fichier PSD prot�g�, retire la protection.
...
//	printf("Mode: %d\n", (int)nMode);
	...
	// Color mode data section. Pour le mode "2", toujours 768 bytes, non entrem�l�s => 256 R, 256 G, 256 B.
...
/*
	...
	// Allocation d'un buffer pour l'image (0 = RAW non compress�).
	u8	*pPlanesBuf;
...
//	// Allocation du gros buffer de d�compression (w*h*nb planes*3(R+G+B)).
//	if ((pPic->pPlanes = (u8 *)malloc(pPic->nNbPlanes * pPic->nWidth * pPic->nHeight * 3)) == NULL)
	// Allocation du gros buffer de d�compression (w*h*nb planes).
	...
	u8	*pBytesPerLines = pPtr;		// Un ptr sur une table de ht * nb de plans de u16, contenant le nombre d'octets � d�packer sur chaque ligne.
	u8	*pSrc = pPtr + (pPic->nHeight * pPic->nNbPlanes * 2);	// Ensuite, les datas des lignes.

	// Plan 0 : w*h*R / w*h*G / w*h*B
	// Plan 1 : w*h*R / w*h*G / w*h*B
	// etc...
	// En mode index�, 1 seul w*h (et pas R puis G puis B).
