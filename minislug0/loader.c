...
// + Fonctions de r�cup�ration de hauteur des blocs, ce genre de trucs.
//
...
//=============================================================================
// Le syst�me de sprites "durs".
// Le truc est pr�vu uniquement pour des objets dont la base est au sol !
...
s32 HardSpr_AddToList(u32 nSpr, s32 nPosX, s32 nPosY, u32 nFlags)
{
	...
	// On r�cup�re le descripteur de sprite.
...
/*
// Fonctionne, mais revoir pour des niveaux de plateforme. Il faudra la m�me chose pour le BlockGetHeight() (� l'envers bien s�r) mais surtout revoir la routine SideCheck() dans le d�placement du h�ros.
...
// Out : 0 => Rien, sinon 1>16.
	// pr�voir un code "dur" qui renvoie 16 imm�diatement (?).
...
// Fonction pour r�cup�rer la hauteur jusqu'au sol � partir d'un point (x,y).
...
s32 BlockGetGroundLevel2(s32 nPixPosX, s32 nPixPosY)
{
	...
	if (nHt == 0)
	{
		...
		// On descend jusqu'� trouver du dur.
		while (nHt == 0)
		{
			nBlkY++;
	...
	// On monte jusqu'� trouver un bloc pas rempli jusqu'en haut.
	while (nHt == 16)
	{
		nBlkY--;
...
// Encapsulation de la fct pr�c�dente + test du hard sprite.
...
//nNbIter++;
		...
		// Les bornes se sont rejointes. Il ne reste qu'un �l�ment � tester. C'est le bon ?
		if (nLength == 0)
		{
...
//			return mid
			...
			// Au m�me Y, choix sur le X.
//			if (pCurPath[nMid].nPosY == nPosY && pCurPath[nMid].nPosX == nPosX)		// le test du Y doit �tre inutile...
...
//printf("Found2: Nb iter=%d\n", nNbIter);
			...
			// Optim possible, sur le m�me Y, rechercher s�quentiellement.
...
// R�cup�ration d'un bloc de path au sol.
...
// R�cup�ration d'un bloc de path en l'air.
...
// Lecture du fichier, calcul du checksum et comparaison avec le checksum sauvegard�.
// Out: 1 = Ok / 0 = Failed.
...
// Lib�re les ressources utilis�es par le niveau en cours.
void LevelRelease(void)
{
	...
	// Lib�re les ressources des monstres.
...
/*
//>> Ok, mais inutile, les monstres sont enregistr�s de gauche � droite et de haut en bas.
...
// Diff�rents types de sections du fichier EDT.
enum
{
	e_FileSection_Map = 0,
	e_FileSection_Plane,
...
// Pragma pack 1 sur les structures � lire/�crire.
#pragma pack(1)
...
*/
// Struct pour datas misc. (Hum, corrig�e et padd�e correctement pour pack).
struct SMisc0
{
	u32	nFlags;						// gMisc.nFlags.
	u8	nTransR, nTransG, nTransB;	// RGB de la couleur de transparence.
...
// In : No du level. On lit le fichier "lev<no>.edt" dans le r�pertoire "lev<no>".
void LevelLoad(u32 nLevelNo)
{
	...
	u32	nPlaneNext = 0;		// Pour controler l'ordre de lecture des plans. Doit �tre dans l'ordre.
	//char	pFilename[256];
	//CPlane	*pPlane;
	struct SMap0	sMap;
	u32	nPlaneSav_BlkLg, nPlaneSav_BlkHt;	// Lg et Ht de la planche sauv�e dans le EDT.

	char	pDir[256];	// Pour cr�er le nom du r�pertoire, � ajouter avant les noms de fichiers.
	char	pLevFilename[256];	// Nom du fichier, pr�c�d� du r�pertoire.
	char	pFilename[256];		// Noms de fichiers annexes � lire (planches...).
...
*/
	...
	// Cr�ation du nom du r�pertoire et du nom de fichier.
...
printf("lev fn: %s\n", pLevFilename);
	...
	// R�cup�ration de la taille du fichier. (Note : Les containers sont lus un par un, donc on alloue d�j� trop, mais c'est pas bien grave).
	s32	nFileSzToLoad;
	fseek(fPt, 0L, SEEK_END);
	nFileSzToLoad = ftell(fPt);
	fseek(fPt, 0L, SEEK_SET);
...
//			(int)((SMap0 *)pBuf)->nPlanesNb, (int)((SMap0 *)pBuf)->nFirstWnd, (int)((SMap0 *)pBuf)->nActivePlane);
			...
			// Par d�faut, position de d�part du joueur.
...
//			(int)((SPlane2 *)pBuf)->nWndPosX, (int)((SPlane2 *)pBuf)->nWndPosY);
			...
			// On va s�parer les chemins de type air et les chemins de type sol.
			if (((struct SPlane2 *)pBuf)->nHasCmd)
			{
				...
				{
					// Au passage, note la position de d�part du h�ros.
					if (*(((s32 *)pCur) + j) == MAP_BLK_HERO)
					{
						...
						gMap.nHeroPlane = gMap.nPlanesNb - 1;	// -1 car le plan des monstres est le plan suivant ! Donc gMap.nPlanesNb a d�j� �t� incr�ment�.
#ifdef DEBUG_INFO
...
*/
			...
			// On r�cup�re les datas qui seront utiles plus tard.
...
/*
// C'�tait �a qui fonctionnait � l'origine.
			// Lecture du BMP.
			...
			SDL_FreeSurface(pGfx2);	// Lib�re le gfx 8 bits.
*/
...
//sans pitch				memcpy((u8 *)pGfx2->pixels, pPlanche->pPlanes, pPlanche->nWidth * pPlanche->nHeight);
				...
				// Lib�re les ressources du PSD.
				//free(pPlanche->pPlanes);		// > On garde la planche en m�moire pour l'alpha, trait� plus bas.
				//free(pPlanche);
			}
			else
			{
			...
			// On v�rifie que la taille de la planche soit bien �gale � celle sauvegard�e.
...
//fprintf(stderr, "prev lg=%d ht=%d / new lg=%d ht=%d\n", nPlaneSav_BlkLg, nPlaneSav_BlkHt, (pGfx2->w / 16), (pGfx2->h / 16));
			...
			// Une page de codes est pr�sente ?
...
//printf("ht : ");
							for (ri = 0; ri < 16; ri++)
							{
								...
								// Plafond ? On fait un NOT sur la hauteur (On sauve les hauteurs du n�gatif du bloc).
...
*/
			...
			// Si n�cessaire, lib�re les ressources du PSD.
			if (pPlanche != NULL)
			{
...
/*
//> v.1.0 - ok. Il faut rajouter la d�tection des tailles des plans.
			// Lecture des blocs.
			for (j = 0; j < gMap.nMapLg * gMap.nMapHt; j++)
			{
				*(*(gMap.ppPlanesBlocks + gMap.nPlanesNb) + j) = *(s32 *)pCur;	// < On pourrait tester les n�s de blocs ici (c�d que �a ne d�passe pas le n� maximum du bloc de la planche).
...
*/
			...
			// Le prochain qui arrivera devrait �tre le...
			nPlaneNext++;
			break;

		case e_FileSection_Cmd :		// On rajoute le fichier de commandes au dernier plan ajout�.
#ifdef DEBUG_INFO
			printf("Cmd section\n");
#endif
			if (nMapSection == 0)
...
*/
			...
			// u32 : Nb de monstres.
			// x : Datas des monstres, les uns � la suite des autres : SMst0 + x bytes de data.
			j = *(u32 *)pCur;
			pCur += 4;
...
//***			memcpy(gLoadedMst.pMstData, pCur, (pBuf + nSz) - pCur);	// on rajoute l'index => plus de memcpy possible...
			...
			// Ce n� d'ordre sera utile pour l'acc�s � la table pMstState. Avec deux listes de ptrs sur les datas, c'est obligatoire.
			{
				u8	*pSrcPt, *pDstPt;
				u32	nb, m;
...
/*
// shoot
			// Initialisation des pointeurs. On en profite pour retourner la liste : de bas en haut (et de droite � gauche, mais c'est pas important).
...
//(int)((SMst0 *)pCur)->nMstNo,(int)((SMst0 *)pCur)->nNbBytes);
			...
			// Compare le fichier de commandes charg� et celui sur disque. Et fait ce qu'il faut.
...
*/
	...
	memset(gMap.pBlkAnmMem, -1, gMap.nPlanesNb * gMap.nMapLg * gMap.nMapHt);	// Tout � 0xFF.
